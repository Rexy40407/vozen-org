...
check(app.includes("id: 'tracker'"), 'the Helper tracker must remain a first-class panel route');
check(app.includes("value === '/tracker'"), 'the Helper tracker hash must resolve to its own route');
check(app.includes('<HelperTracker'), 'the tracker route must render the operational tracker');
check(app.includes('Operational snapshot for the selected server'), 'tracker copy must explain its server scope');
check(foundation.includes('.helper-tracker'), 'the tracker needs its dedicated responsive styles');
