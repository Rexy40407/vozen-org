...
const destination = resolve(repositoryRoot, 'site/panel/helper');
...
if (!destination.startsWith(approvedDestinationRoot)) {
  throw new Error('Refusing to install outside site/panel/helper.');
}
...
if (!output.includes('/panel/helper/')) {
  throw new Error('The Helper build does not use the /panel/helper/ production base.');
}
if (/\.map(?:["']|$)|sourceMappingURL/i.test(output)) {
  throw new Error('The Helper build must not publish source maps.');
}

console.log('Installed Helper panel into site/panel/helper/.');
