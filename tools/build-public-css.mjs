...
const navCss = fs.readFileSync(path.join(cssRoot, 'global-nav-v1.css'), 'utf8').replace(/\r\n?/g, '\n');
const mainCss = fs.readFileSync(path.join(cssRoot, 'main-v43.css'), 'utf8').replace(/\r\n?/g, '\n');
// Windows needs this face to render country flags rather than the letters GB/PT.
const flagFontCss = mainCss.match(/@font-face\s*\{[^}]*Twemoji Country Flags[^}]*\}/)?.[0];
const criticalFontCss = [...(navCss.match(/@font-face\s*\{[^}]*\}/g) || []), flagFontCss]
  .filter(Boolean).join('\n');
